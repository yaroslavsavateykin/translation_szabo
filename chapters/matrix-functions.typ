...
После этих определений остаётся задача вычисления $bb(A)^(1 slash 2)$
или $exp (bb(A))$. Если $bb(A)$ — диагональная матрица,
...
Что делать, если $bb(A)$ не диагональна? Поскольку $bb(A)$ эрмитова,
всегда можно найти унитарное преобразование, диагонализующее её, т. е.
...
Следовательно, чтобы вычислить любую функцию эрмитовой матрицы
$bb(A)$, сначала диагонализуют $bb(A)$ и получают $bb(a)$ —
диагональную матрицу, содержащую все собственные значения $bb(A)$.
Затем вычисляют $f (bb(a))$, что просто, потому что $bb(a)$
диагональна. Наконец, $f (bb(a))$ «раздиагонализуют» с помощью
@eq:1113b, получая @eq:1115. Например, квадратный корень из матрицы
$bb(A)$ можно найти как
...
$f (a_i) eq 1 / a_i eq oo$, и поэтому $bb(A)^(-1)$ не существует. Как
показывает упражнение 1.12(a), определитель матрицы равен произведению
её собственных значений. Следовательно, если одно из собственных
значений $bb(A)$ равно нулю, то $det (bb(A))$ равен нулю, и
...
#task(
  ...
  + Если $bb(G) (omega) eq (omega bb(1) minus bb(A))^(-1)$, то
    ...
    Покажите, что с использованием нотации Дирака это можно переписать
    как
