...
#wrap-content(
  ...
  [Интересный пример использования волновых функций в UHF даёт
    ...
    методом RHF спиновая плотность $rho^S lr((bold(r)))$ всюду
...
#align(center)[
  #figure(
    table(
      ...
      [#strong[$a^C$]],
      [#strong[$a^H$]],
      [#strong[$lr(chevron.l hat(S)^2 chevron.r)$]],
...
дублет с $chevron.l hat(S)^2 chevron.r eq S lr((S plus 1)) eq 3 / 4$.
...
#align(center)[
  #figure(caption: [SCF-расчёты для основного состояния $upright(N)_2$
    ...
    #table(
      ...
      [$upright(N)_2^plus lr((zws^2 Pi_u))$],
      [-108.37855],
      [0.564 (0.624)],

      table.hline(stroke: 0.5pt),
      [$upright(N)_2^plus lr((zws^2 Sigma_g))$],
      [-108.36597],
      [0.576 (0.573)],
      table.hline(stroke: 1pt),
    )
