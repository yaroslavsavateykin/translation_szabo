...
$ tilde(Phi) arrow.r tilde(Phi) plus delta tilde(Phi) $ <eq:325>
...
$ chevron.l Psi_i|Psi_j chevron.r eq S_(i j) $ <eq:334>
...
$ [a|b] minus delta_(a b) eq 0 $ <eq:337>
...
$ chi_a arrow.r chi_a plus delta chi_a $ <eq:342>
...
$ bb(U)^dagger eq bb(U)^(minus 1) $ <eq:651>
...
$ det lr((bb(A) bb(B))) eq det lr((bb(A))) det lr((bb(B))) $ <eq:355>

детерминант преобразованных спин-oрбиталей связан с детерминантом
исходных спин-орбиталей соотношением

$ det lr((bb(A) prime)) eq det lr((bb(U))) det lr((bb(A))) $ <eq:356>
...
$ bb(U)^dagger bb(U) eq bold(1) $ <eq:358>
...
$ det lr((bb(U))) eq e^(i phi.alt) $ <eq:360>
...
$ hat(f) prime lr((1)) eq hat(f) lr((1)) $ <eq:364>
