...
получающийся вектор в общем случае отличается от $|alpha chevron.r$.
Если $hat(O)|alpha chevron.r$ является просто постоянной, умноженной
на $|alpha chevron.r$, т. е.
...
невырождены. Два собственных вектора $|1 chevron.r$ и $|2 chevron.r$
являются вырожденными, если им соответствует одно и то же собственное
значение:
...
Существует много способов найти две линейные комбинации $|1 chevron.r$
и $|2 chevron.r$, которые ортогональны. Одна из таких процедур
называется ортогонализацией Шмидта. Предположим, что $|1 chevron.r$ и
$|2 chevron.r$ нормированы, и пусть
...
Таким образом, собственные векторы $brace.l|alpha chevron.r brace.r$
эрмитова оператора можно выбрать так, чтобы они образовывали
...
базисе $brace.l|i chevron.r brace.r$ в общем случае не диагонально.
Однако его матричное представление в базисе, образованном его
собственными векторами, диагонально. Чтобы показать это, умножим
уравнение для собственного значения @eq:172 слева на $chevron.l beta|$
и используем соотношение ортонормированности @eq:181:
