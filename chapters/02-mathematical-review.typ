...
Более того, мы предполагаем, что собственные функции $hat(H)$ образуют полный набор, и любая функция $lr(bar.v tilde(Phi)) chevron.r$, удовлетворяющая тем же граничным условиям, что и набор $brace.l lr(bar.v Phi_alpha) chevron.r brace.r$, может быть представлена как линейная комбинация $lr(bar.v Phi_alpha) chevron.r$

$ lr(|tilde(Phi)) chevron.r eq sum_alpha lr(bar.v Phi_alpha) chevron.r c_alpha eq sum_alpha lr(bar.v Phi_alpha) chevron.r chevron.l Phi_alpha| tilde(Phi) chevron.r $ <eq:1146>
...
#task()[
  ...
  чтобы показать, что $minus 1 / pi$ является оценкой сверху для точной
...
#task()[
  ...
  чтобы показать, что $minus frac(4, 3pi) eq minus 0.4244$ является оценкой
...
#task()[
  ...
  собственному значению $bb(O)$. Для симметричной матрицы $2 times 2$ ($O_12 eq O_21$)
...
$ |tilde(Phi) chevron.r eq sum_(i eq 1)^N c_i|Psi_i chevron.r $ <eq:1153>
...
$brace.l |Psi_i chevron.r brace.r$ — матрица $bb(H)$ размера
...
но $frac(partial L, partial c_N)$ _не обязательно_ равен нулю. Однако в
...
$ 
где использовались @eq:1154 и @eq:1166. Чтобы понять значимость значений $E$, рассмотpим
...
#task()[
  ...
  $|tilde(Phi) prime chevron.r$, ортогональную точной волновой
  ...
  #block[
    ...
    + Когда $x$ и $y$ выбраны так, что
      $|tilde(Phi) prime chevron.r$ нормирована и выполняeтся
      услoвие $chevron.l tilde(Phi) prime|Phi_0 chevron.r eq 0$,
...
$hat(H)$ в конечном базисе $brace.l|Psi_i chevron.r brace.r$, т.е. $lr((bb(H)))_(i j) eq chevron.l Psi_i lr(|hat(H)|) Psi_j chevron.r$
...
Умножая @eq:1175 слева нa $chevron.l Phi_i|$ и заменяя $cal(E)$
