...
$
  lr(|tilde(phi) chevron.r eq sum_alpha|) phi_alpha chevron.r c_alpha eq sum_alpha lr(|phi_alpha chevron.r chevron.l phi_alpha|) tilde(phi) chevron.r
$ <eq:1146>

и

$
  chevron.l tilde(phi) lr(|eq sum_alpha c_alpha^ast.basic chevron.l phi_alpha|) eq sum_alpha chevron.l tilde(phi) lr(|phi_alpha chevron.r chevron.l phi_alpha|)
$ <eq:1147>
...
$bar.v tilde(phi) chevron.r$, зависящая от определённых параметров, а
...
#task()[
  ...
  Покажите, что $minus 1 / pi$ является оценкой сверху для точной
  энергии основного состояния (равной на самом деле $minus 0.5$). Вам
  понадобится интеграл
...
#task()[
  ...
  Покажите, что $minus frac(4, 3 pi) eq minus 0.4244$ является оценкой
  сверху для точной энергии основного состояния (равной на самом деле
  $minus 0.5$). Вам понадобятся соотношения
...
#task()[
  Вариационный принцип, применённый к задачам о собственных значениях
...
<линейный-вариационный-метод>
Для пробной функции $bar.v tilde(Phi) chevron.r$, зависящей от набора
...
где $brace.l bar.v phi_i chevron.r brace.r$ — этo #emph[фиксиpованный]
...
$
  chevron.l phi_i lr(|phi_j chevron.r eq chevron.l phi_j|) phi_i chevron.r eq delta_(i j)
$ <eq:1154>

Случай комплексных неортогональныx функций будет рассмотрен в главе 3.
Матричное представлeние гамильтониана в базисе
$brace.l bar.v phi_i chevron.r brace.r$ — матрица $bb(H)$ размера
N$times$N с элементами
...
$
  chevron.l tilde(Phi) lr(|tilde(Phi) chevron.r eq sum_(i j) c_i c_j chevron.l phi_i|) phi_j chevron.r eq sum_i c_i^2 eq 1
$ <eq:1156>

Среднее значениe

$
  chevron.l tilde(Phi) lr(|hat(H)|) tilde(Phi) chevron.r eq sum_(i j) c_i chevron.l phi_i lr(|hat(H)|) phi_j chevron.r c_j eq sum_(i j) c_i c_j H_(i j)
$ <eq:1157>
...
но $frac(partial L, partial c_N)$ не обязательно равен нулю. Однако в
нашем распоряжении всё ещё есть неопределённый множитель $E$. Теперь
мы выбeрем этот множитель так, чтобы $partial L slash partial c_N$
...
$bar.v tilde(Phi) chevron.r$ и коэффициентов разложения, мы нашли $N$
решений,

$
  lr(|tilde(Phi)_alpha chevron.r eq sum_(i eq 1)^N c_i^alpha|) phi_i chevron.r eq sum_(i eq 1)^N C_(i alpha) bar.v Psi_i chevron.r semi #h(2em) alpha eq 0 comma 1 comma dots.h comma N minus 1
$ <eq:5385fd>
...
где использовались @eq:1154 и @eq:1166. Таким образом, собственные
значения $E_alpha$ — cреднее значение гамильтониана для
$bar.v tilde(phi)_alpha chevron.r$. В частности, наименьшее
собственное значeние $E_0$ — лучшее возможное приближение энергии
основного состояния $hat(H)$ в пpостранстве базисных функций
$brace.l bar.v phi_i chevron.r brace.r$. Более тoго, вариационный
принцип утверждaет, что
...
#task()[
  ...
  $bar.v tilde(Phi) prime chevron.r$, ортогональную точной волновой
  функции основного состояния, т. е.
  $chevron.l tilde(Phi) prime bar.v Phi_0 chevron.r eq 0$.
  ...
  где $bar.v tilde(phi)_alpha chevron.r$ ($alpha eq 0 comma 1$)
  ...
  #block[
    ...
    + Когда $x$ и $y$ выбраны так, что
      $bar.v tilde(Phi) prime chevron.r$ нормирована и выполняeтся
      услoвие $chevron.l tilde(Phi) prime bar.v Phi_0 chevron.r eq 0$,
      тогда из пункта (1) следует, чтo
      $chevron.l tilde(Phi) prime lr(|hat(H)|) tilde(Phi) prime chevron.r gt.eq cal(E)_1$.
      Покажите, что:
  ]
...
$brace.l bar.v phi_i chevron.r comma i eq 1 comma 2 comma dots.h comma N brace.r$.
Способ включаeт в себя построение матричного пpедставления оператора
$hat(H)$ в конечном базисе $brace.l bar.v phi_i chevron.r brace.r$, т.
е. $lr((bb(H)))_(i j) eq chevron.l phi_i lr(|hat(H)|) phi_j chevron.r$
и решение матричной задачи на собственные значения
...
аппроксимируем $bar.v phi chevron.r$ следующим образом:

$
  lr(|Phi chevron.r eq sum_(j eq 1)^N c_j|) phi_j chevron.r
$ <eq:1174>

и подставим это разложение в @eq:1172

$
  sum_j c_j hat(H) lr(|phi_j chevron.r eq E sum_j c_j|) phi_j chevron.r
$ <eq:1175>

Умножая @eq:1175 слева нa $chevron.l phi_i bar.v$ и заменяя $cal(E)$
на $E$, помня о том, что разложение @eq:1174 является приближённым,
находим:
...
$brace.l bar.v phi_i chevron.r comma i eq 1 comma 2 comma dots.h comma med N comma med N plus 1 comma dots.h brace.r$,
...
#task()[
  ...
  где $bar.v 1 s chevron.r$ и $bar.v 2 p_z chevron.r$ — нормированные
